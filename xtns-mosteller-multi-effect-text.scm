...
(script-fu-register "script-fu-multi-effect-text"
            _"_MultiEffect Text..."
            _"Creates a Multi-Effect Text with a drop shadow"
            "Scott Mosteller"
            "Scott Mosteller"
            "2007"
            ""
            ...
            SF-OPTION     _"Effect Type"       '(_"Cubism" _"Oilify" _"Plasma" _"Diffraction" _"No Effect")
...
(if (= raw FALSE)
  (begin
   ...
   (gimp-drawable-set-name lay1 _"frame1")
...
(if (= raw FALSE)
  (begin
   ...
   (gimp-drawable-set-name lay2 _"frame2")
   (gimp-selection-none img)
   (gimp-context-set-background bg-color)
...
(if (= raw FALSE)
  (begin
   ...
   (gimp-drawable-set-name lay3 _"frame3")
   (gimp-selection-none img)
   (gimp-context-set-background bg-color)
...
(if (= raw FALSE)
  (begin
    ...
    (gimp-drawable-set-name lay4 _"frame4")
    (gimp-selection-none img)
    (gimp-context-set-background bg-color)
...
(define (apply-multi-effect-text img
  ...
  (let* ((width (car (gimp-drawable-width logo-layer)))
     ...
     (shadow-layer (car (gimp-layer-new img width height RGBA-IMAGE _"Shadow" sho NORMAL-MODE)))
         (df1 0)
         (df2 0)
         (df3 0)
         (df4 0)
